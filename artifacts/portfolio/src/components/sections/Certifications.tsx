...
type Certification = { title: string; issuer: string; detail: string; credential: string; verifyUrl?: string };
const certifications: Certification[] = [
  { title: "CCNA", issuer: "Cisco Networking Academy", detail: "98% Score · 120 hrs", credential: "Networking & Security" },
...
export function Certifications() {
  ...
  return <section id="certifications" className="relative py-24 md:py-32" data-testid="certifications-section">
    <div className="max-w-7xl mx-auto px-6">
      <div className="text-center mb-14"><span className="text-sm font-mono uppercase tracking-widest text-primary">05. Credentials</span><h2 className="text-4xl md:text-5xl font-black mt-4">Certified expertise</h2><p className="text-lg mt-4 text-muted-foreground">16+ accredited milestones across engineering, AI, networking, and software.</p></div>
    ...
    <AnimatePresence>{selected && <Dialog open onOpenChange={(open) => !open && setSelected(null)}><DialogContent className="max-w-md glass-card"><DialogHeader><DialogTitle className="flex items-center gap-2"><Award className="text-primary" />{selected.title}</DialogTitle></DialogHeader><div className="space-y-4 text-sm"><p className="text-muted-foreground">Issued by {selected.issuer}</p><div className="rounded-xl bg-primary/10 p-4"><p className="font-semibold">{selected.credential}</p><p className="text-primary mt-1">{selected.detail}</p></div>{selected.verifyUrl ? <a className="inline-flex items-center gap-2 text-primary font-medium" href={selected.verifyUrl} target="_blank" rel="noopener noreferrer">Verify credential <ExternalLink size={14}/></a> : <p className="text-muted-foreground">Credential verification is available on request.</p>}</div></DialogContent></Dialog>}</AnimatePresence>
  </section>;
}
