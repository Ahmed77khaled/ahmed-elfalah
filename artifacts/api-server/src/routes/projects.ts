...
router.post("/", requireAuth, run(async (req, res) => { const parsed = insertProjectSchema.safeParse(req.body); if (!parsed.success) return void fail(res, "Invalid project", 400); ok(res, await cmsService.createProject(parsed.data), 201); }));
router.put("/:id", requireAuth, run(async (req, res) => { const projectId = id(req.params.id); if (projectId === null) return void fail(res, "Invalid project id", 400); const parsed = insertProjectSchema.partial().safeParse(req.body); if (!parsed.success) return void fail(res, "Invalid project", 400); ok(res, await cmsService.updateProject(projectId, parsed.data)); }));
