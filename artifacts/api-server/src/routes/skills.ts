...
router.post("/", requireAuth, run(async (req, res) => { const parsed = insertSkillSchema.safeParse(req.body as any); if (!parsed.success) return void fail(res, "Invalid skill", 400); ok(res, await cmsService.createSkill(parsed.data), 201); }));
router.put("/:id", requireAuth, run(async (req, res) => { const skillId = id(req.params.id); if (skillId === null) return void fail(res, "Invalid skill id", 400); const parsed = insertSkillSchema.partial().safeParse(req.body as any); if (!parsed.success) return void fail(res, "Invalid skill", 400); ok(res, await cmsService.updateSkill(skillId, parsed.data)); }));
